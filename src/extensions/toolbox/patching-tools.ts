import { KcPatching, KcPatchingOptions } from "@src/types/toolbox/patching-tools";
...
function patchString(data: string, opts: KcPatchingOptions = {}): string | false {
 ...
 *
*/
async function patch(filename: string, ...opts: KcPatchingOptions[]): Promise<string | false> {
  return update(filename, (data: string) => {
    const result = opts.reduce(
      (updatedData: string, opt: KcPatchingOptions) => patchString(updatedData, opt) || updatedData,
      data,
    )
