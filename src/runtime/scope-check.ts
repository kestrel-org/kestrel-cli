...
function scopeCheck(toolbox: Toolbox, command: Command) {
  ...
  if (command.scope == 'in') {
    if (project_def === null) {
      ...
      error(
        `The '${command.name}' command must be run inside a kestrel-cli project`
      )
    ...
    if (def_content.projects.hasOwnProperty('backend_path')) {
      if (
        !isDirectory(path.join(root_dir, def_content.projects.backend_path))
      ) {
        error(
          `Cannot find directory '${def_content.projects.backend_path}', did you change its name, if so update it in the kestrel-cli.json file`
        )
        return process.exit(0)
      }
      const backend_path = path.join(
        root_dir,
        def_content.projects.backend_path
      )
    ...
    if (def_content.projects.hasOwnProperty('frontend_path')) {
      if (
        !isDirectory(path.join(root_dir, def_content.projects.frontend_path))
      ) {
        error(
          `Cannot find directory '${def_content.projects.frontend_path}', did you change its name, if so update it in the kestrel-cli.json file`
        )
        return process.exit(0)
      }
      const frontend_path = path.join(
        root_dir,
        def_content.projects.frontend_path
      )
  ...
  if (command.scope == 'out') {
    if (project_def != null) {
      ...
      error(
        `The '${command.name}' command must be run outside a kestrel-cli project`
      )
      return process.exit(0)
    }
  }
