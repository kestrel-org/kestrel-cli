...
const command : Command = {
  ...
  options: [
    {
      flags: '--env <env>',
      description: 'Run in specified env. Values are [dev,prod,test] default to dev.',
      default : 'dev'
    },
    {
      flags: '--args <args>',
  ...
  run: async (toolbox,options,args,command) => {
    ...
    if(command_name == "generate:models"){
      ...
      env = lowerCase(env)
      runCmd = "node"
      sqlzArgs = `node_modules/sequelize-auto/bin/sequelize-auto -o \"./src/models\" -d ${process.env[env_vars[0]]} -h localhost -u ${process.env[env_vars[1]]} -p 3306 -x ${process.env[env_vars[2]]} -e mysql --skipTables sequelizemeta --noInitModels`
      command_name = "generate-models"
    }else{
      runCmd = "npx"
      options.args = options.args ? options.args : ""
      sqlzArgs = `sequelize-cli ${command_name} ${options.args}`
      command_name = `${command_name} ${options.args}`
    }
    toolbox.loader.start(infoLoader(`Running sequelize command : ${command_name}`))
