...
const command : Command = {
  ...
  run: async (toolbox) => {
    ...
    const responses = await prompts.any<SwaggerPrompts>(getPrompts(models,swaggerModels))

    const file_path = path.join(swaggerModels,upperFirst(responses.model)+'.js');
    const props = buildTemplateProperties(responses.model,database.default);

    if(responses.swaggerOverwrite!==undefined && responses.swaggerOverwrite==false)
      return undefined
