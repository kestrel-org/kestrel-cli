  ...
  run: async (toolbox, options, args, command) => {
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          await simpleGit().clone(
            'https://github.com/kestrel-org/kestrel.git',
            `${__dirname}/../templates/angular-node`,
            {}
          )
    ...
    for (let files of templateFiles) {
      generators = files.toTransform.reduce((res, file) => {
        const generator = generate({
          ...
          target: `${props.name}/${
            (single ? file.createPath : file.part) +
            file.filename.replace('.ejs', '')
          }`,
          props: props,
        })
    ...
    if (props.backend_path != null) {
      toolbox.loader.start(
        infoLoader(
          `Installing ${path.basename(
            path.join(cwf, props.backend_path)
          )} dependencies`
        )
      )
    ...
    if (props.frontend_path != null) {
      toolbox.loader.start(
        infoLoader(
          `Installing ${path.basename(
            path.join(cwf, props.frontend_path)
          )} dependencies`
        )
      )
