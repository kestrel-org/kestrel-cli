...
const command : Command = {
  ...
  run: async (toolbox,options,args,command) => {
    ...
    if(cors_conf){
      ...
      let is_present = (back_cors as BackCors).whitelist.some(address => url == address)

      if(!is_present){
        back_cors.whitelist = back_cors.whitelist.concat([url])
      }
    
      await writeAsync(`${back_cors_path}`,`export default ${util.inspect(back_cors)}`,{jsonIndent:4})
      await patch(
        front_cors_path, 
    ...
    if(key_certificate.length!=1){
      exit(command,"Found zero or multiple .key files, did you put it in the sslcert folder ?")
    }
    let crt_certificate = await findAsync(path.join(root_dir,"sslcert"),{
      matching : '*.crt'
    })
    if(crt_certificate.length!=1){
      exit(command,"Found zero or multiple .crt files, did you put it in the sslcert folder ?")
    }
    ...
    await saveLog.run({
      command : "ng",
      args: `build --configuration=production --output-path=${path.relative(frontend_path,path.join(build_dir,"src/public"))}`,
      action : buildExist,
      target : build_dir,
      options : {
        cwd : frontend_path
