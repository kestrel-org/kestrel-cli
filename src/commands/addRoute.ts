...
import { AddRoutePrompts, Database, PathObject, PromptsModels, Route } from '@src/types/commands/addRoute';
...
const {
  generateSwaggerFile,generateTestFile,generateRouterFile,generateServiceFiles
} =  generateUtils
...
const addRoute : Command = {
  ...
  run: async (toolbox,options,args,command) => {
    ...
    if(router_name){
      ...
      do {
        router_name = await prompts.ask("Name of the router ?",undefined,"example/my-router")
    ...
    // @ts-ignore
    const routes : Route[] = (await import(url.pathToFileURL(path.join(routes_folder,"routes.js")))).default
    ...
    // @ts-ignore
    const database : Database = (await import(url.pathToFileURL(path.join(models_folder,"index.js")))).default
    ...
    const responses = await prompts.any<AddRoutePrompts>(getPrompts(router_name, models, questionsPaths));
    ...
    const props = buildTemplateProperties(responses.model, database, responses.path, path_to_model, path_to_app)
    ...
    await generateRouterFile(props,responses.model,router_file_path)
    ...
    if(responses.createService && (responses.serviceOverwrite===undefined || responses.serviceOverwrite)){
      ...
      await generateServiceFiles(props,responses.model,frontPaths)
    }
    
    //  Modify routes.js file

    const new_routes = updateRoutes(routes,router_name,Object.fromEntries(Object.entries(responses).slice(0,3)))
